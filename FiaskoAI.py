...
def login_main():
    ...
    class TelegramLoginDialog(QDialog):
        def __init__(self):
            ...
            self.main_frame.setGeometry(0, 0, 340, 270)
            ...
            self.close_btn.setStyleSheet("color: #ff9800; border: none; font-size: 18px; font-weight: bold;")
            ...
            self.send_code_btn = QPushButton("Выслать код", self.main_frame)
            ...
            self.login_btn = QPushButton("Авторизироваться", self.main_frame)
        ...
        def send_code(self):
            ...
            try:
                ...
                if not api_id or not api_hash:
                    print("API_ID или API_HASH пустые")
                    return
        ...
        def finish_login(self):
            ...
            conf["API_ID"] = int(self.api_id_input.text().strip())
            conf["API_HASH"] = self.api_hash_input.text().strip()

            save_config(conf)

            print("Успешно авторизован! config.json обновлён.")
...
def run_main():
    global app, loop, client, db, conf
    conf = load_config() 
...
class ChatItem(QFrame):
    ...
    def initUI(self):
        ...
        self.name_lbl.setFixedWidth(105)
        ...
        if not self.is_add_button:
            ...
            self.more_btn.setFixedSize(37, 30)
...
class MainApp(QWidget):
    ...
    def toggle_ui_language(self):
        ...
        current = conf.get("LANGUAGE", "en")
        new_lang = "ru" if current == "en" else "en"
        conf["LANGUAGE"] = new_lang

        save_config(conf)

        self.lang_btn.setText(new_lang.upper())
        self.update_ui_language()
        self.refresh_chats()

    def update_ui_language(self):
        self.conn_btn.setText(
            tr("disconnect") if self.is_running else tr("connect")
        )
    ...
    def initUI(self):
        ...
        self.lang_btn = QPushButton(conf.get("LANGUAGE", "en").upper(), self.main_frame)
        self.lang_btn.setGeometry(255, 18, 45, 34)
        self.lang_btn.setStyleSheet("background-color: #2d2d2d; color: white; border-radius: 17px; font-size: 14px; font-weight: bold; border: 1px solid #555;")
        self.lang_btn.clicked.connect(self.toggle_ui_language)
        ...
        self.conn_btn = QPushButton(tr("connect"), self.main_frame)
    ...
    def open_chat_menu(self, user_id):
        ...
        del_btn = QPushButton(tr("delete_chat"))
        ...
        write_btn = QPushButton(tr("write_first"))
        ...
        sync_btn = QPushButton(tr("sync_chat"))
        ...
        sum_btn = QPushButton(tr("summary"))
        ...
        async def run():
            try:
                ...
                signals.update_summary_signal.emit(
                    "Sync complete." if conf.get("LANGUAGE") == "en"
    ...
    def render_chat_list(self, users):
        ...
        add_item = ChatItem(0, tr("add_chat"), "", 0, is_add_button=True, parent_app=self)
